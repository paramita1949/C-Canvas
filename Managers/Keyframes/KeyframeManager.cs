    ...
    public class KeyframeManager
    {
        ...
        public async Task<bool> AddKeyframeAsync(int imageId, double position, int yPosition)
        {
            ...
            if (result > 0)
            {
                ...
                Console.WriteLine($"✅ 添加关键帧成功: 图片{imageId}, Y位置{yPosition}");
                return true;
            }

            Console.WriteLine($"❌ 添加关键帧失败: 该位置附近已存在关键帧");
            return false;
        }
        ...
        public async Task<bool> DeleteKeyframeAsync(int keyframeId, int imageId)
        {
            ...
            if (success)
            {
                ...
                Console.WriteLine($"✅ 删除关键帧成功: ID={keyframeId}");
            }
        ...
        public async Task<bool> ClearKeyframesAsync(int imageId)
        {
            ...
            if (success)
            {
                ClearCache(imageId);
                ScheduleUiUpdate("both");
                Console.WriteLine($"✅ 清除所有关键帧成功: 图片{imageId}");
            }
        ...
        {
            var isBackward = CurrentKeyframeIndex >= 0 &&
                            targetIndex >= 0 &&
                            targetIndex < CurrentKeyframeIndex;

            if (isBackward)
            {
                Console.WriteLine($"🔙 检测到回跳：从关键帧 {CurrentKeyframeIndex + 1} 跳转到关键帧 {targetIndex + 1}");
            }
        ...
        public void SmoothScrollTo(double targetPosition)
        {
            _mainWindow.Dispatcher.Invoke(() =>
            {
                try
                {
                    ...
                    {
                        Console.WriteLine("❌ ScrollViewer为空，无法执行平滑滚动");
                        return;
                    }
                    ...
                    // 执行平滑滚动动画
                    Utils.AnimationHelper.AnimateScroll(
                        scrollViewer,
                        currentPosition,
                        targetOffset,
                    ...
                    Console.WriteLine($"🎬 开始平滑滚动: {currentPosition:F0} → {targetOffset:F0}");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"❌ 平滑滚动异常: {ex.Message}");
                }
            });
        }
