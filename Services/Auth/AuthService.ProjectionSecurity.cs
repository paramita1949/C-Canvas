...
namespace ImageColorChanger.Services
{
    public partial class AuthService
    {
        ...
        private bool ValidateAuthTokens()
        {
            ...
            {
#if DEBUG
                System.Diagnostics.Trace.WriteLine($" [AuthService] 令牌完整性验证失败");
#endif
                return false;
            }
        ...
        public bool CanUseProjection()
        {
            ...
            {
                System.Diagnostics.Trace.WriteLine($" [AuthService] 令牌验证失败，拒绝投影");
            }

            if (decision.Reason == Auth.AuthProjectionAccessPolicy.DenyReason.Expired)
            {
                System.Diagnostics.Trace.WriteLine($" [AuthService] 账号已过期");
        ...
        private DateTime GetEstimatedServerTime()
        {
            ...
            {
#if DEBUG
                System.Diagnostics.Trace.WriteLine(" [AuthService] TickCount 异常（可能系统重启），执行基准重建");
                if (result.LocalTimeRolledBack)
                {
                    System.Diagnostics.Trace.WriteLine(" [AuthService] 检测到时间回退，强制使用正向流逝");
                }
#endif
